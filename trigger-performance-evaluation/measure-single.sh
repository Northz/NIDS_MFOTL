...
trigger_time=$(cat ./out-native.txt | grep diff | sed "s/diff: //" | awk '{n += $1}; END{print n}')
...
echo "$1;$time_rewritten;$rewritten_loop_time;$time_native;$native_loop_time;$trigger_time"
