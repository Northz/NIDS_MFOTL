...
| Trigger: "S, E \<turnstile> \<phi> \<Longrightarrow> S, E \<turnstile> \<psi> \<Longrightarrow> S, E \<turnstile> Formula.Trigger \<phi> \<I> \<psi>" 
| Release: "S, E \<turnstile> \<phi> \<Longrightarrow> S, E \<turnstile> \<psi> \<Longrightarrow> S, E \<turnstile> Formula.Release \<phi> \<I> \<psi>" 
...
named_theorems wty_formulaD "Destruction rules for @{term wty_formulaD}"

lemma wty_formula_NegD [wty_formulaD]:
  assumes "S, E \<turnstile> Formula.Neg \<phi>"
  shows "S, E \<turnstile> \<phi>"
  using assms by cases

lemma wty_formula_OrD [wty_formulaD]:
  assumes "S, E \<turnstile> Formula.Or \<phi> \<psi>"
  shows "S, E \<turnstile> \<phi>" "S, E \<turnstile> \<psi>"
  using assms by (cases, simp)+

lemma wty_formula_AndD [wty_formulaD]:
  assumes "S, E \<turnstile> Formula.And \<phi> \<psi>"
  shows "S, E \<turnstile> \<phi>" "S, E \<turnstile> \<psi>"
  using assms by (cases, simp)+

lemma wty_formula_AndsD [wty_formulaD]:
  assumes "S, E \<turnstile> Formula.Ands \<phi>s" 
    and "\<phi> \<in> set \<phi>s"
  shows "S, E \<turnstile> \<phi>"
  using assms by (cases, simp)

lemma wty_formula_ExistsD [wty_formulaD]:
  assumes "S, E \<turnstile> Formula.Exists t \<phi>"
  shows "S, case_nat t E \<turnstile> \<phi>"
  using assms by cases

lemma wty_formula_PrevD [wty_formulaD]:
  assumes "S, E \<turnstile> Formula.Prev I \<phi>"
  shows "S, E \<turnstile> \<phi>"
  using assms by cases

lemma wty_formula_NextD [wty_formulaD]:
  assumes "S, E \<turnstile> Formula.Next I \<phi>"
  shows "S, E \<turnstile> \<phi>"
  using assms by cases

lemma wty_formula_SinceD [wty_formulaD]:
  assumes "S, E \<turnstile> Formula.Since \<phi> I \<psi>"
  shows "S, E \<turnstile> \<phi>" "S, E \<turnstile> \<psi>"
  using assms by (cases, simp)+

lemma wty_formula_UntilD [wty_formulaD]:
  assumes "S, E \<turnstile> Formula.Until \<phi> I \<psi>"
  shows "S, E \<turnstile> \<phi>" "S, E \<turnstile> \<psi>"
  using assms by (cases, simp)+

lemma wty_formula_TriggerD [wty_formulaD]:
  assumes "S, E \<turnstile> Formula.Trigger \<phi> I \<psi>"
  shows "S, E \<turnstile> \<phi>" "S, E \<turnstile> \<psi>"
  using assms by (cases, simp)+

lemma wty_formula_ReleaseD [wty_formulaD]:
  assumes "S, E \<turnstile> Formula.Release \<phi> I \<psi>"
  shows "S, E \<turnstile> \<phi>" "S, E \<turnstile> \<psi>"
  using assms by (cases, simp)+

lemma wty_formula_TPD [wty_formulaD]:
  assumes "S, E \<turnstile> Formula.TP t"
  shows "E \<turnstile> t :: TInt"
  using assms by cases

lemma wty_formula_TSD [wty_formulaD]:
  assumes "S, E \<turnstile> Formula.TS t"
  shows "E \<turnstile> t :: TInt"
  using assms by cases
...
proof -
  ...
  next 
    ...
    thus ?case 
      by (simp add: wty_formula.Since)
  next
    case (Until S E \<phi> \<psi> I)
    thus ?case
      by (simp add: wty_formula.Until)
  next 
    case (Trigger S E \<phi> \<psi> I)
    thus ?case 
      by (simp add: wty_formula.Trigger)
  next
    case (Release S E \<phi>)
    thus ?case 
      by (simp add: wty_formula.Release)
  next 
    case (MatchP S E r I)
 ...
 next 
  ...
  next
    ...
    hence "S((p, Formula.nfv \<phi>) \<mapsto> tabulate E 0 (Formula.nfv \<phi>)), E' \<turnstile> \<psi>" 
      using LetPast(4)[OF aux wf] by fastforce
    then show ?case using LetPast(1) by(auto intro!: wty_formula.intros) 
  next
    case (TS S E t)
    then show ?case 
      using wty_trm_fv_cong fvi.simps(22) wty_formula.TS
      by blast
  next
    case (TP S E t)
    then show ?case 
      using wty_trm_fv_cong fvi.simps(21) wty_formula.TP 
      by blast
  qed 
  with assms show ?thesis by auto
qed
...
| "sat' \<sigma> V v i (Formula.Trigger \<phi> I \<psi>) = (\<forall>j\<le>i. (mem I (\<tau> \<sigma> i - \<tau> \<sigma> j)) \<longrightarrow> (sat' \<sigma> V v j \<psi> \<or> (\<exists>k \<in> {j <.. i}. sat' \<sigma> V v k \<phi>)))"
| "sat' \<sigma> V v i (Formula.Release \<phi> I \<psi>) = (\<forall>j\<ge>i. (mem I (\<tau> \<sigma> j - \<tau> \<sigma> i)) \<longrightarrow> (sat' \<sigma> V v j \<psi> \<or> (\<exists>k \<in> {i ..< j}. sat' \<sigma> V v k \<phi>)))"
...
proof(induction i arbitrary:xs rule: less_induct)
  ...
  let ?V' = "(V((p, length xs) \<mapsto> \<lambda>a b. b < x \<and> letpast_sat (\<lambda>X v i. sat' \<sigma> (V((p, length xs) \<mapsto> X)) v i \<phi>) a b))"
  {
    ...
    hence "wty_event (S((p, Formula.nfv \<phi>) \<mapsto> ?tys)) pa xsa"
...
proof -
  ...
  let ?V' = "(V((p, Formula.nfv \<phi>) \<mapsto> letpast_sat (\<lambda>X v i. sat' \<sigma> (V((p, Formula.nfv \<phi>) \<mapsto> X)) v i \<phi>)))"
  {
    ...
    hence "wty_event (S((p, Formula.nfv \<phi>) \<mapsto> ?tys)) pa xs"
...
lemma wty_formula_TT [simp]: "S, E \<turnstile> Formula.TT"
  by (auto simp: Formula.TT_def 
      intro: wty_formula.intros wty_trm.intros)

lemma sat'_TT_FF[simp]:
  shows "sat' \<sigma> V v i Formula.TT" 
    and "\<not> sat' \<sigma> V v i Formula.FF"
  unfolding Formula.TT_def Formula.FF_def 
  by auto

lemma sat'_eventually[simp]: "sat' \<sigma> V v i (eventually I \<phi>) 
  \<longleftrightarrow> (\<exists>j\<ge>i. mem I (\<tau> \<sigma> j - \<tau> \<sigma> i) \<and> sat' \<sigma> V v j \<phi>)"
  by (auto simp: eventually_def)

lemma sat'_always[simp]: "sat' \<sigma> V v i (always I \<phi>) 
  \<longleftrightarrow> (\<forall>j\<ge>i. mem I (\<tau> \<sigma> j - \<tau> \<sigma> i) \<longrightarrow> sat' \<sigma> V v j \<phi>)"
...
lemma sat'_release_rewrite_0_mem:
  fixes i j :: nat
  assumes mem: "mem I 0"
    and trigger: "sat' \<sigma> V v i (Formula.Release \<phi> I \<psi>)"
    and geq: "j\<ge>i"
    and mem_j: "mem I (\<tau> \<sigma> j - \<tau> \<sigma> i)"
  shows "sat' \<sigma> V v j \<psi> 
  \<or> (\<exists>k \<in> {i ..< j}. mem I (\<tau> \<sigma> k - \<tau> \<sigma> i) \<and> sat' \<sigma> V v k (Formula.And \<phi> \<psi>))"
proof (cases "\<exists>k \<in> {i..<j}. sat' \<sigma> V v k \<phi>")
  case True
  define A and k 
    where "A = {x \<in> {i..<j}. sat' \<sigma> V v x \<phi>}" 
      and "k = Min A"
  have A_props: "A \<noteq> {}" "finite A" 
    using True A_def by auto
  hence k_in_A: "k \<in> A" 
    using k_def by auto
  hence k_props: "k \<in> {i..<j}" "sat' \<sigma> V v k \<phi>" 
    by (auto simp: A_def)
  have "(\<forall>l<k. l \<notin> A)"
    using Min_le[OF A_props(2)]
    by (fastforce simp: k_def)
  moreover have "mem I (\<tau> \<sigma> k - \<tau> \<sigma> i)" 
    using mem mem_j k_props interval_leq[of I 0 \<sigma> j i k] by auto
  ultimately show ?thesis 
    using k_props mem trigger by (auto simp: A_def)
next
  case False
  then show ?thesis 
    using assms by auto
...
lemma sat'_until_true:
  assumes "\<not>mem I 0"
  shows "sat' \<sigma> V v i (Formula.Until \<phi> I Formula.TT) 
  \<longleftrightarrow> sat' \<sigma> V v i (Formula.Until \<phi> I (Formula.Prev all \<phi>))"
proof (rule iffI)
  assume "sat' \<sigma> V v i (Formula.Until \<phi> I Formula.TT)"
  then obtain j 
    where j_props: "j>i"  "mem I (\<tau> \<sigma> j - \<tau> \<sigma> i)" "\<forall>k\<in>{i..<j}. sat' \<sigma> V v k \<phi>" 
    using le_eq_less_or_eq assms 
    by auto
...
next
  ...
  thus "sat' \<sigma> V v i (Formula.Until \<phi> I Formula.TT)" 
    by auto
qed
...
lemma sat'_always_rewrite_0:
  ...
  shows "sat' \<sigma> V v i (always I \<phi>) \<longleftrightarrow> sat' \<sigma> V v i (always_safe_0 I \<phi>)"
proof (rule iffI)
  assume all: "sat' \<sigma> V v i (always I \<phi>)"
  {
    define A and j 
      where "A = {j. j\<ge>i \<and> mem (flip_int_double_upper I) (\<tau> \<sigma> j - \<tau> \<sigma> i)}"
        and "j = Inf A"
    assume "\<exists>j\<ge>i. mem (flip_int_double_upper I) (\<tau> \<sigma> j - \<tau> \<sigma> i)"
    hence "A \<noteq> {}" 
      using A_def by auto
    hence "j \<in> A" 
      using j_def by (simp add: Inf_nat_def1)
    hence j_props: "j\<ge>i" "mem (flip_int_double_upper I) (\<tau> \<sigma> j - \<tau> \<sigma> i)" 
      using A_def by auto
    {
      fix k
      assume k_props: "k \<in> {i..<j}"
      hence ineq: "\<tau> \<sigma> k - \<tau> \<sigma> i \<le> \<tau> \<sigma> j - \<tau> \<sigma> i" by (simp add: diff_le_mono)
      {
        assume "mem (flip_int_double_upper I) (\<tau> \<sigma> k - \<tau> \<sigma> i)"
        hence "k \<in> A" using A_def k_props by auto
        hence "k \<ge> j" using j_def cInf_lower by auto
        hence "False" using k_props by auto
      }
      hence "\<not>mem (flip_int_double_upper I) (\<tau> \<sigma> k - \<tau> \<sigma> i)" by blast
      hence "mem I (\<tau> \<sigma> k - \<tau> \<sigma> i)"
        using j_props ineq assms(1) flip_int_double_upper_leq[of I "(\<tau> \<sigma> j - \<tau> \<sigma> i)" "(\<tau> \<sigma> k - \<tau> \<sigma> i)"]
        by auto 
      hence "sat' \<sigma> V v k \<phi>" using k_props all by auto
    }
    hence until_j: "\<forall>k\<in>{i..<j}. sat' \<sigma> V v k \<phi>" by blast
    have "\<not> mem (flip_int_double_upper I) 0"
      using assms
      by (simp add: flip_int_double_upper.rep_eq memL.rep_eq)
    hence "sat' \<sigma> V v i (Formula.Until \<phi> (flip_int_double_upper I) (Formula.Prev all \<phi>))"
  ...
  moreover {
    ...
    have "B \<noteq> {}" 
      using assms(2) B_def bounded_memR 
      by auto
    hence "b \<in> B" 
      using b_def 
      by (simp add: Inf_nat_def1)
    hence b_props: "\<not> memR I b" 
      using B_def by auto
    have exists_db: "\<And>x. \<exists>j\<ge>i. x \<le> \<tau> \<sigma> j - \<tau> \<sigma> i"
      using ex_le_\<tau>[of i "_ + \<tau> \<sigma> i" \<sigma>]
      by (meson Nat.le_diff_conv2 add_leE)
    hence "C \<noteq> {}" 
      using C_def by auto
    hence "c \<in> C" 
      using c_def by (simp add: Inf_nat_def1)
    hence c_props: "c\<ge>i" "b \<le> \<tau> \<sigma> c - \<tau> \<sigma> i" 
      using C_def by auto
    have "b \<noteq> 0" and b_pos: "b>0"
      using b_props memR_nonzeroD
      by (meson assms(1) | presburger)+
    have "c \<noteq> i" and c_ge_i: "c>i" 
      using c_props b_pos antisym_conv2  
      by fastforce+
    {
      assume "\<not>mem I (\<tau> \<sigma> (c-1) - \<tau> \<sigma> i)"
      hence "\<not>memR I (\<tau> \<sigma> (c-1) - \<tau> \<sigma> i)" using assms(1) memL_mono by blast
      hence "(\<tau> \<sigma> (c-1) - \<tau> \<sigma> i) \<in> B" using B_def by auto
      hence "(\<tau> \<sigma> (c-1) - \<tau> \<sigma> i) \<ge> b" using b_def cInf_lower by auto
      hence "(c-1) \<in> C" using C_def c_ge_i by auto
      hence "c-1 \<ge> c" using c_def cInf_lower by auto
      hence "False" using c_ge_i by auto
    }
    hence c_pred_mem: "mem I (\<tau> \<sigma> (c-1) - \<tau> \<sigma> i)" 
      by blast
    hence c_pred_sat: "sat' \<sigma> V v (c-1) \<phi>" 
      using all c_ge_i by auto
    {
      assume "\<not>mem (flip_int I) (\<tau> \<sigma> c - \<tau> \<sigma> (c-1))" (* [b+1, \<infinity>] but attention, here 'b' = b+1 *)
      hence "memR I (\<tau> \<sigma> c - \<tau> \<sigma> (c-1))" 
        using assms(1) bounded_memR int_flip_mem by blast
      hence c_dist: "(\<tau> \<sigma> c - \<tau> \<sigma> (c-1)) < b" 
        using b_props memR_antimono not_le by blast
      from c_pred_mem have "memR I (\<tau> \<sigma> (c-1) - \<tau> \<sigma> i)" 
        by auto
      hence "(\<tau> \<sigma> (c-1) - \<tau> \<sigma> i) < b" 
        using b_props memR_antimono not_le by blast
      hence b_ineq: "b \<le> (\<tau> \<sigma> c - \<tau> \<sigma> i)" "(\<tau> \<sigma> c - \<tau> \<sigma> i) \<le> 2 * (b-1)"
        ...
        by auto
      {
        ...
        hence "(b-1) \<in> B" 
          using B_def by auto
        hence "(b-1) \<ge> b" 
          using b_def cInf_lower by auto
        hence "False" 
          using b_pos by linarith
      }
      hence "memR I (b-1)" 
        by blast
      hence "(\<lambda>i. memR I ((div) i 2)) (2*(b-1))" 
        by auto
      hence "(\<lambda>i. memR I ((div) i 2)) (\<tau> \<sigma> c - \<tau> \<sigma> i)" 
        using b_ineq by auto
      hence "memR (flip_int_double_upper I) (\<tau> \<sigma> c - \<tau> \<sigma> i)"
    ...
    hence "mem (flip_int I) (\<tau> \<sigma> c - \<tau> \<sigma> (c-1))" 
      by blast
    hence c_pred_sat: "sat' \<sigma> V v (c-1) (Formula.And \<phi> (Formula.Next (flip_int I) Formula.TT))"
    ...
    hence "\<forall>j\<in>{i..<(c-1)}. sat' \<sigma> V v j \<phi>" using all by auto
    hence
        "(c-1)\<ge>i"
      ...
      by auto
    hence "sat' \<sigma> V v i (Formula.Until \<phi> I (Formula.And \<phi> (Formula.Next (flip_int I) Formula.TT)))"
      by auto
  }
...
next
  ...
  hence "sat' \<sigma> V v i (Formula.Or (Formula.Until \<phi> (flip_int_double_upper I) (Formula.Prev all \<phi>)) (Formula.Until \<phi> I (Formula.And \<phi> (Formula.Next (flip_int I) Formula.TT))))"
    using always_safe_0_def
    by metis
  hence "sat' \<sigma> V v i (Formula.Until \<phi> (flip_int_double_upper I) Formula.TT) \<or> sat' \<sigma> V v i (Formula.Until \<phi> I (Formula.And \<phi> (Formula.Next (flip_int I) Formula.TT)))" by auto
  moreover {
    ...
    then obtain j where j_props:
      ...
      by auto
    hence "\<forall>k\<ge>i. mem I (\<tau> \<sigma> k - \<tau> \<sigma> i) \<longrightarrow> k<j"
      by (metis (no_types, lifting) assms flip_int_double_upper.rep_eq forall_finite(1) interval_leq leI memL.rep_eq prod.sel(1))
    hence "sat' \<sigma> V v i (always I \<phi>)" using j_props by auto
  }
  moreover {
    ...
    then obtain j where j_props:
      ...
      by auto
    hence phi_sat: "\<forall>k\<in>{i..j}. sat' \<sigma> V v k \<phi>" by auto
    {
      fix k
      assume k_props: "k>j" "mem I (\<tau> \<sigma> k - \<tau> \<sigma> i)"
      hence t_geq: "(\<tau> \<sigma> k - \<tau> \<sigma> i) \<ge> (\<tau> \<sigma> (j+1) - \<tau> \<sigma> i)" using diff_le_mono by auto
      from j_props have "mem (flip_int I) (\<tau> \<sigma> (j+1) - \<tau> \<sigma> j)" by auto
      hence "\<not>mem I (\<tau> \<sigma> (j+1) - \<tau> \<sigma> i)"
        by (metis \<tau>_mono assms(2) diff_le_mono2 
            flip_int.rep_eq j_props(1) memL.rep_eq memL_mono prod.sel(1))
      hence "\<not>memR I (\<tau> \<sigma> (j+1) - \<tau> \<sigma> i)" 
        using assms memL_mono by blast
      hence "\<not>memR I (\<tau> \<sigma> k - \<tau> \<sigma> i)"
        using t_geq memR_antimono by blast
      hence "False" 
        using k_props by auto
    }
    hence "\<forall>k>j. \<not>mem I (\<tau> \<sigma> k - \<tau> \<sigma> i)" 
      by auto
    hence "sat' \<sigma> V v i (always I \<phi>)" 
      using phi_sat by auto
  }
  ultimately show "sat' \<sigma> V v i (always I \<phi>)" 
    by blast
qed
...
proof (rule iffI)
  assume release: "sat' \<sigma> V v i (Formula.Release \<phi> I \<psi>)"
  {
    ...
    hence "sat' \<sigma> V v i (Formula.Or (Formula.Until \<psi> I (Formula.And \<phi> \<psi>)) (always I \<psi>))" 
      by auto 
  }
  moreover {
    assume "\<not>(\<forall>j\<ge>i. mem I (\<tau> \<sigma> j - \<tau> \<sigma> i) \<longrightarrow> sat' \<sigma> V v j (Formula.And \<psi> (Formula.Neg \<phi>)))"
    then obtain j' where j'_props: "j' \<ge> i" "mem I (\<tau> \<sigma> j' - \<tau> \<sigma> i)" 
      "\<not> sat' \<sigma> V v j' (Formula.And \<psi> (Formula.Neg \<phi>))" 
      by blast
    ...
    from j'_props have "\<not>sat' \<sigma> V v j' \<psi> \<or> sat' \<sigma> V v j' \<phi>" 
      by simp
    moreover {
      assume "\<not>sat' \<sigma> V v j' \<psi>"
      hence "\<exists>k \<in> {i..<j'}. mem I (\<tau> \<sigma> k - \<tau> \<sigma> i) \<and> sat' \<sigma> V v k (Formula.And \<phi> \<psi>)"
      using j'_props assms release sat'_release_rewrite_0_mem[of I \<sigma> V v i \<phi> \<psi> j']
      by auto
    hence A_props: "A \<noteq> {} \<and> finite A" using A_def by auto
    hence "j \<in> A" using j_def by auto
    hence j_props: "j \<in> {i..<j'} \<and> mem I (\<tau> \<sigma> j - \<tau> \<sigma> i) \<and> sat' \<sigma> V v j (Formula.And \<phi> \<psi>)"
      using A_def
      by auto
    {
      ...
      hence "mem I (\<tau> \<sigma> k - \<tau> \<sigma> i)"
        ...
        by auto
      hence "\<exists>x \<in> {i..<k}. mem I (\<tau> \<sigma> x - \<tau> \<sigma> i) \<and> sat' \<sigma> V v x (Formula.And \<phi> \<psi>)"
    ...
    hence "\<forall>k \<in> {i..<j}. sat' \<sigma> V v k \<psi>" by blast
    hence "sat' \<sigma> V v i (Formula.Or (Formula.Until \<psi> I (Formula.And \<phi> \<psi>)) (always I \<psi>))"
      using j_props
      by auto
    }
    moreover {
      ...
      hence B_props: "B \<noteq> {}" "finite B" using B_def j'_props by auto
      hence "k \<in> B" using k_def by auto
      hence k_props: "k\<in>{i..j'}" "mem I (\<tau> \<sigma> k - \<tau> \<sigma> i)" "sat' \<sigma> V v k \<phi>" using B_def by auto
        ...
        by (fastforce simp: k_def)
      {
        ...
        hence l_mem: "mem I (\<tau> \<sigma> l - \<tau> \<sigma> i)"
          using assms k_props interval_leq[of I 0 \<sigma> k i l]
          by auto
        {
          assume "sat' \<sigma> V v l \<phi>"
          hence "l \<in> B" using B_def l_props l_mem k_props by auto
          hence "l\<ge>k" "l<k"
            using k_def l_props B_props(2) Min_le[of B l]
            by auto
        }
        hence "\<not>sat' \<sigma> V v l \<phi>" by auto
      }
      hence not_phi: "\<forall>l\<in>{i..<k}. \<not>sat' \<sigma> V v l \<phi>" using assms B_def by auto
      
      hence k_sat_psi: "sat' \<sigma> V v k \<psi>" using k_props release B_def by auto
      {
        fix l
        assume l_props: "l\<in>{i..<k}"
        hence "mem I (\<tau> \<sigma> l - \<tau> \<sigma> i)"
          using k_props assms interval_leq[of I 0 \<sigma> k i l]
          by auto
        hence "sat' \<sigma> V v l \<psi>"
          using l_props release not_phi
          by auto
      }
      hence "\<forall>l\<in>{i..<k}. sat' \<sigma> V v l \<psi>"
        using not_phi assms release
        by auto
      hence "sat' \<sigma> V v i (Formula.Until \<psi> I (Formula.And \<phi> \<psi>))"
  ...
  hence "sat' \<sigma> V v i (Formula.Or (Formula.Until \<psi> I (Formula.And \<phi> \<psi>)) (always_safe_0 I \<psi>))"
...
next
  ...
  moreover {
    ...
    hence "sat' \<sigma> V v i (Formula.Release \<phi> I \<psi>)" by auto
  }
  moreover {
    fix j
    assume until_and_j_props: "sat' \<sigma> V v i (Formula.Until \<psi> I (Formula.And \<phi> \<psi>))" 
      "j \<ge> i" "mem I (\<tau> \<sigma> j - \<tau> \<sigma> i)"
    then obtain "j'" where j'_props: "j'\<ge>i" "mem I (\<tau> \<sigma> j' - \<tau> \<sigma> i)" 
      "sat' \<sigma> V v j' (Formula.And \<phi> \<psi>)" "(\<forall>k \<in> {i ..< j'}. sat' \<sigma> V v k \<psi>)"
      by fastforce
    moreover {
      assume ge: "j' > j"
      hence "\<forall>k \<in> {i ..< j'}. sat' \<sigma> V v k \<psi>" using j'_props by auto
      hence "sat' \<sigma> V v j \<psi>" using until_and_j_props ge by auto
      hence "sat' \<sigma> V v j \<psi> \<or> (\<exists>k \<in> {i ..< j}. sat' \<sigma> V v k \<phi>)" by auto
    }
    moreover {
      assume leq: "\<not> j' > j"
      moreover {
        assume "j = j'"
        hence "sat' \<sigma> V v j \<psi> \<or> (\<exists>k \<in> {i ..< j}. sat' \<sigma> V v k \<phi>)"
          using j'_props
          by auto
      }
      moreover {
...
next
  ...
  next
    ...
    moreover {
      ...
      hence ?case using ty_of_eval_trm unfolding eval
...
next
  ...
  have "S, E \<turnstile> \<phi>" 
    using wty_formula_AndD 
      \<open>S, E \<turnstile> formula.And \<phi> (formula.Trigger \<phi>' I \<psi>')\<close> 
    by blast
  thus ?case 
    using And_Trigger 
...
next
  ...
  have "S, E \<turnstile> \<phi>" 
    using wty_formula_AndD \<open>S, E \<turnstile> formula.And \<phi> (formula.Release \<phi>' I \<psi>')\<close>
    by blast
  thus ?case 
    using And_Release
    by (auto simp: safe_dual_def split: if_splits)
next
  ...
  proof cases
    ...
    hence "\<psi> \<in> set pos" using Ands(1) by (auto simp add: psidef)
    thus "\<exists>\<phi>\<in>set pos. x \<in>fv  \<phi>" using psidef by auto
  next
    assume " \<not> safe_formula \<psi>"
    hence "\<psi> \<in> set neg" using Ands(1) by (auto simp add: psidef)
...
next
  ...
  have xfv: "x \<in> fv \<psi>"
    using Trigger_0(3,9) 
    by auto
  thus ?case
    using Trigger_0
    by (auto elim: wty_formula.cases elim!: allE[of _ i])
next
...
next
  ...
  have xfv: "x \<in> fv \<psi>"
    using Release_0(4,9)
    by auto
  moreover have "S, E \<turnstile> release_safe_0 \<phi> I \<psi> \<longleftrightarrow> S, E \<turnstile> Formula.Release \<phi> I \<psi>" for S E
    unfolding release_safe_0_def
    by (auto simp: always_safe_0_def 
        intro!: wty_formula.intros dest: wty_formulaD)
  ultimately show ?case
    using sat'_release_rewrite_0[OF Release_0.hyps(1,2), symmetric] Release_0 
    by (auto elim: wty_formula.cases)
next
  case (MatchP I r)
  ...
  have IH: "S, E \<turnstile> \<phi> \<Longrightarrow> \<phi> \<in> safe_atms r \<Longrightarrow> sat' \<sigma> V v j \<phi> 
    ...
    by blast
  show ?case 
    using wty MatchP.prems(4) phidef 
    by (auto intro: IH)
next
  case (MatchF I r)
  ...
  have IH: "S, E \<turnstile> \<phi> \<Longrightarrow> \<phi> \<in> safe_atms r \<Longrightarrow> sat' \<sigma> V v j \<phi> 
    ...
    by blast
  show ?case 
    using wty MatchF.prems(4) phidef
    by (auto intro: IH)
next
...
next
  ...
  have "(safe_letpast p ` regex.atms r) = (safe_letpast p ` regex.atms r')"
    using image_iff rel_regex_regex_atms[OF all_rel] rel_regex_regex_atms'[OF all_rel] 19(1) 
    by auto fastforce+ 
  then show ?case unfolding \<psi>_def by auto
next
  case (20 p I r)
  then obtain r' where 
    ...
    by(cases \<psi>) auto
  have "(safe_letpast p ` regex.atms r) = (safe_letpast p ` regex.atms r')"
    using image_iff rel_regex_regex_atms[OF all_rel] rel_regex_regex_atms'[OF all_rel] 20(1) 
    by auto fastforce+ 
  then show ?case unfolding \<psi>_def by auto
next
  case (21 p t)
...
next
  ...
  proof(cases \<psi>)
    ...
    hence fv: "fv \<psi>' = {}" using orig(1) rel_formula_fv by blast
...
next
  ...
  then show ?case
    by simp
next
  case (Trigger_0 \<phi>' I \<psi>' \<psi>)
  have "formula.rel_formula f (formula.Neg \<gamma>2) \<gamma>1 \<Longrightarrow>
    \<phi>' = formula.Neg \<gamma>2 \<Longrightarrow>
    \<forall>x. formula.rel_formula f \<gamma>2 x \<longrightarrow> safe_formula x \<Longrightarrow>
    \<forall>\<gamma>'. \<gamma>1 = formula.Neg \<gamma>' \<longrightarrow> \<not> safe_formula \<gamma>' \<Longrightarrow> safe_formula \<gamma>1" 
    for \<gamma>1 :: "'b Formula.formula" and \<gamma>2 :: "'a Formula.formula"
    by (cases \<gamma>1; clarsimp)
  thus ?case
    using Trigger_0 
...
next
  case (Release_0 \<phi>' I \<psi>')
  have obs: "formula.rel_formula f \<phi>' \<gamma>1 \<Longrightarrow> formula.rel_formula f \<psi>' \<gamma>2 
    \<Longrightarrow> formula.rel_formula f (release_safe_0 \<phi>' I \<psi>') (release_safe_0 \<gamma>1 I \<gamma>2)" 
    for \<gamma>1 \<gamma>2 :: "'b Formula.formula"
    by (clarsimp simp: release_safe_0_def always_safe_0_def Formula.TT_def)
  show ?case
    using Release_0
    apply (cases \<psi>) 
    using obs
    by (auto simp: case_Neg_iff rel_formula_fv)
...
lemma trm_f_in_fv: assumes  "n \<in> set xs" "E n = t" "\<forall>n' \<in> set xs . E n' = t \<longrightarrow> E2 n' = E2 n "
  ...
  {assume "x = n"
    ...
    hence ?case using asm snoc.prems(2) by auto
    }
...
proof -
  ...
  hence f''_eq: "f' t = (f'' \<circ> new_type_symbol) t" "f' type = f'' (new_type_symbol type)" for t
...
proof - 
  ...
  hence f_def: "f =  update_env (f type, oldt) E" using assms
...
proof -
  ...
  hence f_def: "f = update_env (f (new_type_symbol type), oldt) (new_type_symbol \<circ> E)" using assms(1) 
...
proof -
  ...
  hence f_def: "f =  update_env (f type, oldt) (E)" using assms(1) 
...
proof -
  ...
  proof -
    ...
    hence "g \<circ> (f \<circ> E) \<turnstile> t :: g (f type)" 
...
proof -
  {
    ...
    hence "(\<exists>g. wf_f (TCst \<circ> g) \<and> f'' = (g \<circ> (f' \<circ> f)))"
...
proof -
  ...
  hence constr_int: "constr = TCst TInt \<Longrightarrow> wf_f (TCst \<circ> f'') \<Longrightarrow> resultless_trm_f f'' f' type \<Longrightarrow> f'' type = TInt" for f'' 
  ...
  moreover {
    ...
    hence "\<exists>g. wf_f (TCst \<circ> g) \<and>
...
proof - 
  ...
  hence wf_fp': "wf_f fp'" using wty_result_fX_trm_def assms(1) fp'_def by blast
  ...
  proof -
    ...
    hence "(g \<circ> (fp \<circ> E) \<turnstile> t :: g (TCst b))" 
  ...
  { 
    ...
    hence wty': "(f' \<circ> E) \<turnstile> t :: b" "(f' \<circ> E) \<turnstile> trm t :: a" 
    ...
    hence "(\<exists>g. wf_f (TCst \<circ> g) \<and> f' = (g \<circ> f))" by force 
  }
  moreover 
  {
    fix f'
...
lemma eq_refinement_min_type: 
  assumes "\<exists> f g . wf_f f \<and> wf_f g \<and> f typ = g typ'"
...
fun trivial_inst :: "(tysym \<Rightarrow> ty)"  
  where "trivial_inst (TCst ty) = ty" 
...
proof -
  ...
  hence "clash_propagate constr type_start E_start = error e \<Longrightarrow> False" for e
  ...
  hence wf_end: "wf_f (f' \<circ> final_f)" using assms(6) wf_f_comp by force
  hence half_wty: "half_wty_trm E type (trm t1 t2) (f' \<circ> final_f)"
  ...
  hence g_typ: "g \<circ> (f' \<circ> final_f \<circ> E) \<turnstile> t1 :: g ((f' \<circ> final_f) type)"
  ...
  hence "half_wty_trm (f' \<circ> final_f \<circ> E) ((f' \<circ> final_f) type) (trm t1 t2) f''"
    apply(rule subterm_half_wty) using assms(6) by (auto elim: wty_trm.cases) 
  hence half_wty2: "half_wty_trm (f' \<circ> final_f \<circ> E) ((f' \<circ> final_f) type) (trm t1 t2) f''"
  ...
  hence g'_typ: "g' \<circ> (f'' \<circ> (f' \<circ> final_f) \<circ> E) \<turnstile> t2 :: g' ((f'' \<circ> (f' \<circ> final_f)) type)"
...
proof -
 have cp_none: "clash_propagate (TCst a) type E = error e \<Longrightarrow> False" for e
  ...
  hence "min_type type (TCst a) = return (TCst a, type)"
...
proof (induction t arbitrary: E type f e)
  ...
  hence "min_type (E x) type \<in> range return" 
...
next
  ...
  hence "min_type (TCst (ty_of x)) type \<in> range return" 
...
| "check S E (Formula.Trigger \<phi> I \<psi>) = check_two_formulas check S E \<phi> \<psi>"
| "check S E (Formula.Release \<phi> I \<psi>) = check_two_formulas check S E \<phi> \<psi>"
...
lemma check_binary_sound: 
  assumes "\<And>\<phi>' S E f'. size \<phi>' \<le> size \<phi> + size \<psi> \<Longrightarrow> check S E \<phi>' = return f' 
  \<Longrightarrow> wf_formula \<phi>' \<Longrightarrow> wty_result_fX S E \<phi>' f'"
  "check S E form = return f'" "wf_formula \<phi>" "wf_formula \<psi>"
  "form \<in> {formula.Or \<phi> \<psi>, formula.And \<phi> \<psi>, formula.Since \<phi> I \<psi>, formula.Until \<phi> I \<psi>, 
    formula.Trigger \<phi> I \<psi> , formula.Release \<phi> I \<psi>}" 
...
proof -
  ...
  show ?thesis
    ...
    proof -
      ...
      hence "(map_sig (g \<circ> f1 \<circ> f) S, (g \<circ> f1 \<circ> f) \<circ> E \<turnstile> formula.map_formula (g \<circ> f1 \<circ> f) \<psi>)"
...
proof -
  ...
  have f'_def: "f = f2 \<circ> f1 \<circ> new_type_symbol" using assms(1,3) f1_def f2_def by (auto simp add: check_comparison_def split: option.splits)
  {
    ...
    hence g_def': "wf_f (TCst \<circ> g)" "fa = g \<circ> f1 \<circ> new_type_symbol" 
    ...
    hence "\<exists>g'. wf_f (TCst \<circ> g') \<and> (g \<circ> f1 \<circ> new_type_symbol = g' \<circ> f2 \<circ> f1 \<circ> new_type_symbol)" 
      using g_def wty2 unfolding wty_result_fX_trm_def by auto 
    hence "\<exists>g. wf_f (TCst \<circ> g) \<and> (fa = g \<circ> f)" by (simp add: f'_def fun.map_comp g_def'(2))
  } moreover {
    fix fa
    assume 
      wfa: "wf_f (TCst \<circ> fa)" and
    ...
    hence "(g \<circ> f2 \<circ> f1) \<circ> (new_type_symbol \<circ> E) \<turnstile> t2 :: fa (nn (TAny 0))" 
      using t_eq' by (simp add: comp_assoc)
    hence wt2: "(fa \<circ> nn \<circ> new_type_symbol \<circ> E \<turnstile> t2 :: fa (nn (TAny 0)))"
    ...
    hence "map_sig fa S, fa \<circ> E \<turnstile> formula.map_formula fa form" 
...
proof(induction arbitrary: f S rule:check_pred.induct)
  ...
  hence sr: "map_sig f' S' (r, length trms) = Some (map f' ts)" by (auto simp:map_sig_def)
  ...
  note wty2 = 1(1)[OF f'_def, of "map_sig f' S'"] 
  {
    ...
    hence "fa \<circ> E \<turnstile> trm :: fa t" 
    ...
    hence "trm \<in> set trms \<Longrightarrow> (fa \<circ> E) \<turnstile> trm :: t \<Longrightarrow> (g \<circ> (f' \<circ> E)) \<turnstile> trm :: t" for t trm
      using wty_trm_fv_cong by blast
    hence "list_all2 (wty_trm (g \<circ> (f' \<circ> E))) trms (map fa ts)" 
      using la by auto (metis list.rel_mono_strong)
    hence *: "(map_sig (g \<circ> f') S', (g \<circ> (f' \<circ> E)) \<turnstile> formula.map_formula g (formula.Pred r trms))"
    ...
    have "map_sig (g \<circ> f'' \<circ> f') S', (g \<circ> f'') \<circ> (f' \<circ> E) \<turnstile> (formula.Pred r trms)"
      using wty2 unfolding wty_result_fX_def 
      using wf_comb g_def(1)
      by (smt (verit, best) S'_def f''_def formula.simps(529) 
          map_sig_comp map_sig_subst map_upd_Some_unfold) 
    hence la: "list_all2 (wty_trm (g \<circ> f'' \<circ> f' \<circ> E)) trms (map (g \<circ> f'' \<circ> f') ts)" 
      using sr using wty_formula.intros(1)[of "map_sig (g \<circ> f'' \<circ> f') S'", OF sr] 
      by (auto elim:wty_formula.cases simp:comp_assoc)
    {
      fix trm y
      assume assm: "trm \<in> set trms" "y \<in> fv_trm trm"
      have "(g \<circ> f'' \<circ> f' \<circ> E) y = (f''' \<circ> E) y" 
        using g_def(2) by (simp add: assm(2) subset_iff) 
    }
    hence "trm \<in> set trms \<Longrightarrow> (g \<circ> f'' \<circ> f' \<circ> E) \<turnstile> trm :: t \<Longrightarrow> (f''' \<circ> E) \<turnstile> trm :: t" for t trm
      using wty_trm_fv_cong by blast
    hence "list_all2 (wty_trm (f''' \<circ> E)) trms (map f''' ts)" 
      using wty_trm_fv_cong la list.rel_mono_strong g_def(2) by force
    hence *: "list_all2 (wty_trm (f''' \<circ> E)) (trm#trms) (map f''' (t#ts))" 
...
next
  ...
  hence wty1: "wty_result_fX S E (formula.Ands \<phi>s) f'" 
  ...
  moreover {
    ...
    hence eq: "formula.map_formula g (formula.map_formula f' a) = formula.map_formula fa a"
    ...
    hence "(map_sig g (map_sig f' S), g \<circ> (f' \<circ> E) \<turnstile> formula.map_formula g (formula.map_formula f' a))"
    ...
    hence "\<exists>g. wf_f (TCst \<circ> g) \<and> fa = g \<circ> f" 
    ...
    hence "(map_sig fa S, fa \<circ> E \<turnstile> (formula.Ands (map (formula.map_formula fa) \<phi>s)))" by simp
    hence fst: " \<forall>\<phi>\<in>set (map (formula.map_formula fa) \<phi>s). map_sig fa S, fa \<circ> E \<turnstile> \<phi>" 
    ...
    hence "(map_sig fa S, fa \<circ> E \<turnstile> formula.map_formula fa a)" 
      using ty[unfolded *] by (simp add: f_def fun.map_comp g_def(2))
    hence "(map_sig fa S, fa \<circ> E \<turnstile> formula.map_formula fa (formula.Ands (a # \<phi>s)))"
...
proof -
  ...
  moreover {
    ...
    hence "regex.pred_regex (wty_formula (map_sig fa S) (fa \<circ> E)) (regex.map_regex (formula.map_formula fa) (binop r1 r2))"
    ...
    then obtain g where g_def: "wf_f (TCst \<circ> g)" "fa = g \<circ> f'"
      using wty1 wf unfolding wty_result_fX_def by auto
    hence aux: "(map_sig fa S, g \<circ> (f' \<circ> E) \<turnstile> formula.map_formula fa (type I r2))"
    ...
    hence comp_eq: "(formula.map_formula g \<circ> formula.map_formula f') = formula.map_formula (g \<circ> f')"
      by auto
    ...
    have "(formula.map_formula (g \<circ> f'') \<circ> formula.map_formula f') x = formula.map_formula (g \<circ> f'' \<circ> f') x" for x
      by (simp add: formula.map_comp)
    hence comp_eq: "(formula.map_formula (g \<circ> f'') \<circ> formula.map_formula f') = formula.map_formula (g \<circ> f'' \<circ> f')"
      by auto
    ...
    hence "(map_sig fa S, fa \<circ> E \<turnstile> formula.map_formula fa (type I (binop r1 r2)))" 
      using type binop by(auto intro!:wty_formula.intros)
  }
...
next
  ...
  hence "S, E \<turnstile> formula.map_formula f (formula.MatchP I (regex.Star r)) \<or>
              S, E \<turnstile> formula.map_formula f (formula.MatchF I (regex.Star r))"
    by auto
  hence "Regex.pred_regex (\<lambda>\<phi>. S, E \<turnstile> \<phi>) (regex.Star (regex.map_regex (formula.map_formula f) r))"
    by(auto elim:wty_formula.cases) 
...
next
  case (Test x)
  hence f_def: "check S E x = return f"
    by auto
  have wf: "wf_formula x" using Test(1,3) type by auto
  have wty: "wty_result_fX S E x f" using Test(4)[OF _ f_def wf] by auto
  hence "wf_f f" unfolding wty_result_fX_def by auto
  moreover {
    fix fa
    assume wf: "wf_f (TCst \<circ> fa)" and
           ty: "map_sig fa S, fa \<circ> E \<turnstile> formula.map_formula fa (type I (regex.Test x))"
    hence "map_sig fa S, fa \<circ> E \<turnstile> formula.MatchP I (regex.Test (formula.map_formula fa x)) \<or>
               map_sig fa S, fa \<circ> E \<turnstile> formula.MatchF I (regex.Test (formula.map_formula fa x))"
      using type by auto
    hence "Regex.pred_regex (\<lambda>\<phi>. map_sig fa S, fa \<circ> E \<turnstile> \<phi>) (regex.Test (formula.map_formula fa x))"
       by(auto elim:wty_formula.cases) 
    hence "map_sig fa S, fa \<circ> E \<turnstile> formula.map_formula fa x"
      by auto
    hence "\<exists>g. wf_f (TCst \<circ> g) \<and> fa = (g \<circ> f)" using wty wf unfolding wty_result_fX_def by auto
  } moreover {
    fix fa
    assume wf: "wf_f (TCst \<circ> fa)" and
           exg: "\<exists>g. wf_f (TCst \<circ> g) \<and> (fa = g \<circ> f)"
...
next
  case (Star r)
  hence *: "check S E (type I r) = return f" using type by auto
...
proof (induction S E \<phi> arbitrary: f rule: check.induct)
  ...
  next
    case (Some a)
    hence "check_pred r E ts a = return f" using 1(1) by auto
...
next
  ...
  moreover
  {
    ...
    hence fa'_TAny: "x \<in> fv (formula.map_formula (fa' \<circ> f0) \<phi>) \<Longrightarrow> (fa' \<circ> TAny) x = E' x" for x
    ...
    hence "fa = g2 \<circ> (f2 \<circ> f1 \<circ> f0)"
  ...
  moreover
  {
    ...
    hence "map_sig (g \<circ> f2 \<circ> f1) (map_sig f0 S), (g \<circ> f2 \<circ> f1) \<circ> TAny
    ...
    hence 2: "(map_sig fa S)((p, Formula.nfv \<phi>) \<mapsto> tabulate ((g \<circ> f2 \<circ> f1) \<circ> TAny) 0 (Formula.nfv \<phi>)), fa \<circ> E
...
next
  case (6 S E \<phi>)
  hence "wty_result_fX S E \<phi> f" unfolding used_tys_def by auto 
...
next
  ...
  have wfs: "wf_f f'" "wf_f f''" "wf_f f'''" "wf_f f''''" using wty1 wty2 wty3 wty4 unfolding wty_result_fX_def wty_result_fX_trm_def by auto
  {
    ...
    hence "(g' \<circ> (f'' \<circ> f' \<circ> new_type_symbol \<circ> E)) y = g' ((f'' \<circ> f') (t_res_tysym agg_type (agg_trm_tysym agg_type)))"
      using g_def(2) g'_def(2) eq1 by fastforce
    hence "(g' \<circ> (f'' \<circ> f' \<circ> new_type_symbol \<circ> E)) \<turnstile> trm.Var y :: g' ((f'' \<circ> f') (t_res_tysym agg_type (agg_trm_tysym agg_type)))" 
      using Var by blast
    then obtain g'' where g''_def:
      "wf_f (TCst \<circ> g'')" 
      "g' = g'' \<circ> f'''"
    ...
    hence "(g'' \<circ> (f''' \<circ> f'' \<circ> f' \<circ> new_type_symbol \<circ> E)) y = g'' ((f''' \<circ> f'' \<circ> f' \<circ> new_type_symbol) d)"
    ...
    hence aux: "t \<notin> range new_type_symbol \<Longrightarrow> t = TAny 0 \<or> t = TNum 0" for t 
      by auto
    ...
    qed
    hence fa_eq: "fa \<circ> nn = g \<circ> f'''' \<circ> f''' \<circ> f'' \<circ> f'" using p1 by blast
    hence "(fa \<circ> nn \<circ> (new_type_symbol \<circ> agg_tysenv E tys) \<turnstile> trm :: (fa \<circ> nn) (agg_trm_tysym agg_type))"
      using wty1 unfolding wty_result_fX_trm_def by (metis (full_types) fun.map_comp g_def(1) wf_f_comp wfs(2) wfs(3) wfs(4))
    hence prem1: "(fa \<circ> agg_tysenv E tys) \<turnstile> trm :: (fa \<circ> nn) (agg_trm_tysym agg_type)"
    ...
    hence prem3: "map_sig fa S, agg_env (fa \<circ> E) (map fa tys) \<turnstile> formula.map_formula fa \<phi>"
    ...
    hence "(fa \<circ> E)  \<turnstile> trm.Var y :: (fa \<circ> nn) (t_res_tysym agg_type (agg_trm_tysym agg_type))" 
      by (metis (no_types, opaque_lifting) comp_def f_def fa_eq fun.map_comp g_def(2))
    hence "(fa \<circ> E) y = (fa \<circ> nn) (t_res_tysym agg_type (agg_trm_tysym agg_type))" by(auto elim:wty_trm.cases)
    hence prem4: "(fa \<circ> E) y = t_res agg_type ((fa \<circ> nn) (agg_trm_tysym agg_type))"
    ...
    hence "(fa \<circ> E) \<turnstile> trm.Var y :: fa d" by (simp add: f_def fun.map_comp g_def(2))
    hence prem5: "fa d = t_res agg_type ((fa \<circ> nn) (agg_trm_tysym agg_type))"
...
next
  case (12 S E I \<phi>)
  hence "wty_result_fX S E \<phi> f" unfolding used_tys_def by simp 
  then show ?case unfolding wty_result_fX_def by (auto intro: wty_formula.intros elim: wty_formula.cases) 
next
  case (13 S E I \<phi>)
  hence "wty_result_fX S E \<phi> f" unfolding used_tys_def by simp
...
next
  ...
  show ?case 
    ...
    using 15 by auto
next 
  case (16 S E \<phi> I \<psi>)
  show ?case 
    apply (rule check_binary_sound) 
    using 16 by auto
next 
  case (17 S E \<phi> I \<psi>)
  show ?case 
    apply (rule check_binary_sound)
    using 17 by auto
next 
...
next
  ...
  have wf_sub: "wf_formula \<phi>" "wf_formula \<psi>"
    using 20(4) by auto
  note wty1 = 20(1)[OF f0_def f1_def wf_formula_map[OF wf_sub(1)]]
  note wty2 = 20(2)[OF f0_def f1_def f2_def wf_formula_map[OF wf_sub(2)]]
  have "wf_f f0"
    unfolding f0_def wf_f_def new_type_symbol_pow by auto
  hence "wf_f f"
    unfolding f_def using wf_f_comp wty1 wty2 wty_result_fX_def by auto
...
next
  case (21 S E t)
  hence f_def: "check_trm E (TCst TInt) t = return f" by auto
...
proof(induction arbitrary: f S rule:check_pred.induct)
  case (1 r E trm trms t ts)
  hence wty: "f \<circ> E \<turnstile> trm :: f t" using 1(3) by (auto simp del:comp_apply)
  ...
  hence S': "map_sig f' S' (r, length trms) = Some (map f' ts)" by (auto simp:map_sig_def)
...
lemma check_binary_complete: 
  assumes IH: "\<And>\<phi>' S E f e. size \<phi>' \<le> size \<phi> + size \<psi> \<Longrightarrow> check S E \<phi>' = error e 
    \<Longrightarrow> map_sig f S, f \<circ> E \<turnstile> formula.map_formula f \<phi>'\<Longrightarrow> wf_f (TCst \<circ> f) 
    \<Longrightarrow> wf_formula \<phi>' \<Longrightarrow> False" 
    and none: "check S E form = error e" 
    and wty: "map_sig f S, f \<circ> E \<turnstile> formula.map_formula f form" 
    and wf: "wf_f (TCst \<circ> f)" 
    and wformula: "wf_formula form" 
    and type: "form \<in> {formula.Or \<phi> \<psi>, formula.And \<phi> \<psi>, formula.Since \<phi> I \<psi>, formula.Until \<phi> I \<psi>,
    formula.Trigger \<phi> I \<psi>, formula.Release \<phi> I \<psi>}" 
  shows "False"
proof -
  have wformulas: "wf_formula \<phi>" "wf_formula \<psi>" 
    using wformula type by auto
  ...
  have "wty_result_fX S E \<phi> f'" 
    using check_sound_proven[OF f_def] wformula type by auto
  then obtain g where g_def: "wf_f (TCst \<circ> g)" "f = g \<circ> f'" 
    unfolding wty_result_fX_def using wtys(1) wf by auto
  ...
  show ?thesis 
    using IH[OF _ _ wty' g_def(1)] wformulas(2) f_def none type 
    by(auto simp:check_two_formulas_def split:bind_splits)
qed
...
proof -
  ...
  have wf': "wf_formula (formula.map_formula f' (type I r2))" 
    using wfs(2) type by auto
  obtain g where g_def: "wf_f (TCst \<circ> g)" "f = g \<circ> f'"
    using check_sound_proven[OF f'_def wfs(1)] wf wtys(1) 
    unfolding wty_result_fX_def by auto
  have "(map_sig f S), (g \<circ> f') \<circ> E \<turnstile> formula.map_formula g (formula.map_formula f' (type I r2))"
    unfolding formula.map_comp 
    using g_def(2) type  wtys(2) by blast
  hence wty': 
    "(map_sig g (map_sig f' S)), g \<circ> (f' \<circ> E) \<turnstile> formula.map_formula g (formula.map_formula f' (type I r2))"
    by (simp add: fun.map_comp g_def(2))
  show ?thesis 
    using IH[OF _ _ wty' g_def(1) wf'] f'_def none type binop 
    by (auto split:bind_splits)
qed
...
next
  ...
  hence wty: "map_sig f S, f \<circ> E \<turnstile> formula.map_formula f x" 
    by(auto elim:wty_formula.cases)  
...
next
  ...
  hence wtys: "map_sig f S, f \<circ> E \<turnstile> formula.map_formula f (formula.Ands \<phi>s)" "map_sig f S, f \<circ> E \<turnstile> formula.map_formula f a"
  ...
  hence none: "check (map_sig f' S) (f' \<circ> E) (formula.map_formula f' a) = error e" 
  ...
  hence wty': "map_sig g (map_sig f' S), g \<circ> (f' \<circ> E) \<turnstile> formula.map_formula g (formula.map_formula f' a)"
...
proof -
  ...
  hence wty': "f \<circ> nn \<circ> (new_type_symbol \<circ> E) \<turnstile> t1 :: (f \<circ> nn) (TAny 0)" using wty'' by auto
  ...
  hence g_aux: "g \<circ> (f' \<circ> new_type_symbol \<circ> E) \<turnstile> t2 :: g (f' (TAny 0))"
    by (simp add: fun.map_comp)
...
next
  ...
  hence g_def': "f = g \<circ> f' \<circ> new_type_symbol" by (metis comp_id fun.map_comp nn_n)
...
next
  case (16 S E \<phi> I \<psi>)
  show ?case using check_binary_complete[OF _ 16(2-5), of \<phi> \<psi>] 16(1) by blast
next
  case (17 S E \<phi> I \<psi>)
...
next
  ...
  define fa' where fa'_def: "fa' = (\<lambda>t. (case t of TAny n \<Rightarrow> if n < Formula.nfv \<phi> then E' n else f (TAny (n - Formula.nfv \<phi>)) |
                                                     ...
                                                     _ \<Rightarrow> f t))"
  hence wf_fa': "wf_f (TCst \<circ> fa')"
    using 20(5) unfolding wf_f_def by(auto simp:numeric_ty_def)
...
fa' \<circ> TAny \<turnstile> formula.map_formula fa' (formula.map_formula f0 \<phi>)"
  ...
  then obtain g1 
    where wf_g1: "wf_f (TCst \<circ> g1)" 
      and fa'_alt: "fa' = g1 \<circ> f1"
    using wty1'[unfolded nfv_map] wf_fa' 
    unfolding wty_result_fX_def by blast
  have list_eq: "map (fa' \<circ> TAny) [0..<Formula.nfv \<phi>] = map E' [0..<Formula.nfv \<phi>]"
    unfolding fa'_def by simp
  have wty2': "map_sig g1 (map_sig f1 (map_sig f0 S((p, Formula.nfv \<phi>) \<mapsto> map TAny [0..<Formula.nfv \<phi>]))),
    ...
    by (simp add: tabulate_alt formula.map_comp fa'_f0 map_sig_subst list_eq
          flip: comp_assoc fa'_alt)
  show ?case 
    using 20(2)[OF f0_def f1_def _ wty2' wf_g1 wf_formula_map[OF wfs(2)]] f1_def 20(3)
    by(auto simp del:comp_apply simp:Let_def f0_def split:bind_splits)
next
  case (21 S E t)
...
| "check_wty_formula (Formula.Trigger \<phi> I \<psi>) = (check_wty_formula \<phi> \<and> check_wty_formula \<psi>)"
| "check_wty_formula (Formula.Release \<phi> I \<psi>) = (check_wty_formula \<phi> \<and> check_wty_formula \<psi>)"
