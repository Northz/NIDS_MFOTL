...
fun valid_mmsaux :: "args \<Rightarrow> ts \<Rightarrow> 'a mmsaux \<Rightarrow> 'a msaux \<Rightarrow> bool" where
  "valid_mmsaux args cur (nt, gc, maskL, maskR, data_prev, data_in, tuple_in, tuple_since) ys \<longleftrightarrow>
    ...
    newest_tuple_in_mapping ts_tuple_rel data_in tuple_in (\<lambda>x. valid_tuple tuple_since x) \<and>
...
proof -
  ...
  then show ?thesis
    ...
    by (auto simp only: valid_mmsaux.simps shift_end.simps Let_def newest_tuple_in_mapping_def
        split: prod.splits)
       auto
      (* takes long *)
qed
