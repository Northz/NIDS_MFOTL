...
proof (rule iffI)
  ...
  moreover {
    ...
    moreover {
      assume "\<not>mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> k)"
        ...
        moreover {
          ...
          moreover {
            ...
            moreover {
              ...
              then have "mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> (c+1))" using assms by auto
...
lemma sat_trigger_rewrite_bounded:
  ...
  assumes "memL I2 = (\<le>) 0" "memR I2 = (\<lambda>i. enat i \<le> (a-1))"
  assumes "memL I3 = (\<le>) 0" "memR I3 = (\<lambda>i. enat i \<le> (b-a))"
  assumes "a>0"
shows "sat \<sigma> V v i (Trigger \<phi> I1 \<psi>) = sat \<sigma> V v i (Or (Or (historically I1 \<psi>) (once I2 \<phi>)) (once I2 (Prev all (Since \<psi> all (And \<phi> \<psi>)))))"
proof (rule iffI)
  assume trigger: "sat \<sigma> V v i (Trigger \<phi> I1 \<psi>)"
