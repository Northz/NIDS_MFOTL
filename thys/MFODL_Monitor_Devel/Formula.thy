...
lemma historically_rewrite_unbounded:
  ...
  assumes "a > 0"
  shows "sat \<sigma> V v i (And (once I1 \<phi>) (historically I1 \<phi>)) = sat \<sigma> V v i (Or (Since \<phi> I1 (And \<phi> first)) (And (once I3 (Prev all (Since \<phi> all (And \<phi> first)))) (once I1 \<phi>)))"
proof (rule iffI)
  ...
  then have j_props: "j\<le>i \<and> mem I1 (\<tau> \<sigma> i - \<tau> \<sigma> j) \<and> sat \<sigma> V v j \<phi>" using A_def by auto
  {
    ...
    then have "mem I1 (\<tau> \<sigma> i - \<tau> \<sigma> k)" using j_props assms(1-2) by auto
  ...
  moreover {
    define B where "B = {k. k\<le>i \<and> mem I3 (\<tau> \<sigma> i - \<tau> \<sigma> k) \<and> \<not>sat \<sigma> V v k \<phi>}"
    define k where "k = Min B"
    assume "\<exists>j\<le>i. mem I3 (\<tau> \<sigma> i - \<tau> \<sigma> j) \<and> \<not>sat \<sigma> V v j \<phi>"
    then have "B \<noteq> {}" using B_def by auto
    moreover have "finite B" using B_def by auto
    ultimately have B_props: "B \<noteq> {} \<and> finite B" by auto
    then have k_in_B: "k \<in> B" using k_def by auto
    then have k_props: "k\<le>i \<and> mem I3 (\<tau> \<sigma> i - \<tau> \<sigma> k) \<and> \<not>sat \<sigma> V v k \<phi>" using B_def by auto
    {
      fix x
      assume x_props: "x<k" "mem I3 (\<tau> \<sigma> i - \<tau> \<sigma> x)"
      moreover {
        assume "x \<in> B"
        then have "False" using x_props k_def B_props by auto
      }
      moreover {
        assume "x \<notin> B"
        then have "sat \<sigma> V v x \<phi>" using B_def x_props k_props by auto
      }
      ultimately have "sat \<sigma> V v x \<phi>" by blast
    }
    then have less_k: "\<forall>x<k. mem I3 (\<tau> \<sigma> i - \<tau> \<sigma> x) \<longrightarrow> sat \<sigma> V v x \<phi>" by blast
    {
      fix x
      assume x_props: "x<k" "mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> x)" "\<not>mem I3 (\<tau> \<sigma> i - \<tau> \<sigma> x)"
      then have "\<tau> \<sigma> i - \<tau> \<sigma> x = a" using assms(3-6) by auto
      then have "mem I1 (\<tau> \<sigma> i - \<tau> \<sigma> x)" using assms(1-2) by auto
      then have "sat \<sigma> V v x \<phi>" using historically x_props k_props by auto
    }
    then have less_k: "\<forall>x<k. mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> x) \<longrightarrow> sat \<sigma> V v x \<phi>" using less_k by blast
    moreover {
      assume k_pre_props: "k > 0 \<and> mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> (k-1))"
      {
        fix x
        assume x_props: "x\<le>(k-1)"
        {
          assume "mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> x)"
          then have "sat \<sigma> V v x \<phi>" using x_props less_k k_pre_props by auto
        }
        moreover {
          assume "\<not>mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> x)"
          then have "mem I1 (\<tau> \<sigma> i - \<tau> \<sigma> x)" using assms by auto
          then have "sat \<sigma> V v x \<phi>" using x_props k_props historically by auto
        }
        ultimately have "sat \<sigma> V v x \<phi>" by auto
      }
      then have "\<forall>x\<le>(k-1). sat \<sigma> V v x \<phi>" by auto
      then have "sat \<sigma> V v i (once I3 (Prev all (Since \<phi> all (And \<phi> first))))"
        using interval_all less_k k_pre_props k_props
        by (auto split: nat.split)
    }
    moreover {
      assume "\<not>(k > 0 \<and> mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> (k-1)))"
      then have k_or: "k \<le> 0 \<or> \<not>mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> (k-1))" by auto
      moreover {
        assume "k\<le>0"
        then have "False" using k_props assms leq_j j_props by blast
      }
      moreover {
        assume k_pos: "k>0"
        then have "\<not>mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> (k-1))" using k_or by auto
        then have k_pre: "mem I1 (\<tau> \<sigma> i - \<tau> \<sigma> (k-1))" using assms by auto
      ...
      ultimately have "sat \<sigma> V v i (once I3 (Prev all (Since \<phi> all (And \<phi> first))))" by blast
    }
    ultimately have "sat \<sigma> V v i (once I3 (Prev all (Since \<phi> all (And \<phi> first))))" by blast
    then have "sat \<sigma> V v i (once I3 (Prev all (Since \<phi> all (And \<phi> first)))) \<and> sat \<sigma> V v i (once I1 \<phi>)"
      using historically
      by auto
    then have "sat \<sigma> V v i (And (once I3 (Prev all (Since \<phi> all (And \<phi> first)))) (once I1 \<phi>))" by auto
  }
  moreover {
    assume interval_2_sat: "\<forall>j\<le>i. mem I3 (\<tau> \<sigma> i - \<tau> \<sigma> j) \<longrightarrow> sat \<sigma> V v j \<phi>"
    {
      fix k
      assume k_props: "k \<in> {j<..i}"
      {
        assume "mem I3 (\<tau> \<sigma> i - \<tau> \<sigma> k)"
        then have "sat \<sigma> V v k \<phi>" using interval_2_sat k_props by auto
      }
      moreover {
    ...
    then have geq_j: "\<forall>k \<in> {j<..i}. sat \<sigma> V v k \<phi>" by auto
    have "\<tau> \<sigma> 0 \<le> \<tau> \<sigma> j" by auto
    then have "(\<tau> \<sigma> i - \<tau> \<sigma> 0) \<ge> (\<tau> \<sigma> i - \<tau> \<sigma> j)" by linarith
    then have "mem I1 (\<tau> \<sigma> i - \<tau> \<sigma> 0)" using assms j_props by auto
    then have "sat \<sigma> V v i (Since \<phi> I1 (And \<phi> first))" using leq_j geq_j interval_all by auto
  }
  ultimately have "sat \<sigma> V v i (Since \<phi> I1 (And \<phi> first)) \<or> sat \<sigma> V v i (And (once I3 (Prev all (Since \<phi> all (And \<phi> first)))) (once I1 \<phi>))" by blast
  then show "sat \<sigma> V v i (Or (Since \<phi> I1 (And \<phi> first)) (And (once I3 (Prev all (Since \<phi> all (And \<phi> first)))) (once I1 \<phi>)))" by auto
next
  define A where "A = {j. j\<le>i \<and> mem I1 (\<tau> \<sigma> i - \<tau> \<sigma> j)}"
  assume rewrite: "sat \<sigma> V v i (Or (Since \<phi> I1 (And \<phi> first)) (And (once I3 (Prev all (Since \<phi> all (And \<phi> first)))) (once I1 \<phi>)))"
  then have "sat \<sigma> V v i (Since \<phi> I1 (And \<phi> first)) \<or> sat \<sigma> V v i (And (once I3 (Prev all (Since \<phi> all (And \<phi> first)))) (once I1 \<phi>))" by auto
  moreover {
    assume "sat \<sigma> V v i (Since \<phi> I1 (And \<phi> first))"
    then have "sat \<sigma> V v i (And (once I1 \<phi>) (historically I1 \<phi>))"
      using interval_all
      by auto
  }
  moreover {
    assume once: "sat \<sigma> V v i (And (once I3 (Prev all (Since \<phi> all (And \<phi> first)))) (once I1 \<phi>))"
    then have "sat \<sigma> V v i (once I3 (Prev all (Since \<phi> all (And \<phi> first))))" by auto
    then have "\<exists>j\<le>i. mem I3 (\<tau> \<sigma> i - \<tau> \<sigma> j) \<and> sat \<sigma> V v j (Prev all (Since \<phi> all (And \<phi> first)))"
      by auto
    then obtain j where j_props: "j\<le>i \<and> mem I3 (\<tau> \<sigma> i - \<tau> \<sigma> j) \<and> sat \<sigma> V v j (Prev all (Since \<phi> all (And \<phi> first)))" by blast
    {
      assume "j = 0"
    ...
    then have "(case j of 0 \<Rightarrow> False | Suc x \<Rightarrow> mem all (\<tau> \<sigma> j - \<tau> \<sigma> x) \<and> sat \<sigma> V v x (Since \<phi> all (And \<phi> first)))"
      using j_props
      by auto
      ...
      by (simp add: Nitpick.case_nat_unfold)
    {
      ...
      then have "mem I3 (\<tau> \<sigma> i - \<tau> \<sigma> x)" using j_props assms(4-6) by auto
      then have "\<not>mem I1 (\<tau> \<sigma> i - \<tau> \<sigma> x)" using assms by auto
    }
    ...
    then have "sat \<sigma> V v i (And (once I1 \<phi>) (historically I1 \<phi>))"
      using interval_all once
      by auto
  }
  ultimately have "sat \<sigma> V v i (And (once I1 \<phi>) (historically I1 \<phi>))" by blast
...
lemma sat_trigger_rewrite_0_mem:
  ...
  shows "sat \<sigma> V v j \<psi> \<or> (\<exists>k \<in> {j <.. i}. sat \<sigma> V v k (And \<phi> \<psi>))"
proof (cases "\<exists>k \<in> {j<..i}. sat \<sigma> V v k \<phi>")
  case True
  define A where "A = {x \<in> {j<..i}. sat \<sigma> V v x \<phi>}"
  define k where "k = Max A"
  from True A_def have A_nonempty: "A \<noteq> {}" by auto
  from A_def have A_fin: "finite A" by auto
  from k_def A_nonempty A_fin have k_in_A: "k \<in> A" by auto
  then have k_props: "k \<in> {j<..i}" "sat \<sigma> V v k \<phi>" by (auto simp: A_def)
  have "(\<forall>l>k. l \<notin> A)"
    using Max_ge[OF A_fin]
    by (fastforce simp: k_def)
  moreover have "mem I (\<tau> \<sigma> i - \<tau> \<sigma> k)" using mem mem_j k_props interval_geq[of I 0 \<sigma> i j k] by auto
...
lemma sat_trigger_rewrite_0_exist:
  assumes mem: "mem I 0"
  assumes trigger: "sat \<sigma> V v i (Trigger \<phi> I \<psi>)"
  assumes j_props: "j\<le>i \<and> mem I (\<tau> \<sigma> i - \<tau> \<sigma> j) \<and> \<not>sat \<sigma> V v j \<psi>"
shows "\<exists>k \<in> {j<..i}. mem I (\<tau> \<sigma> i - \<tau> \<sigma> k) \<and> sat \<sigma> V v k (And \<phi> \<psi>)"
proof -
  from assms have "\<exists>k\<in>{j<..i}. sat \<sigma> V v k (And \<phi> \<psi>)"
    using sat_trigger_rewrite_0_mem[of I \<sigma> V v i \<phi> \<psi> j]
    by auto
  then obtain k where k_props: "k \<in> {j<..i} \<and> sat \<sigma> V v k (And \<phi> \<psi>)" by blast
  from this mem j_props have "mem I (\<tau> \<sigma> i - \<tau> \<sigma> k)"
    using interval_geq[of I 0 \<sigma> i j k]
    by auto
  from this k_props show ?thesis by auto
qed

lemma sat_trigger_rewrite_0_equiv:
  assumes mem: "mem I 0"
...
proof (rule iffI)
  ...
  moreover {
    ...
    then have "\<exists>k \<in> {j'<..i}. mem I (\<tau> \<sigma> i - \<tau> \<sigma> k) \<and> sat \<sigma> V v k (And \<phi> \<psi>)"
      using mem trigger sat_trigger_rewrite_0_exist[of I \<sigma> V v i \<phi> \<psi> j']
      by auto
    then have A_nonempty: "A \<noteq> {}" using A_def by auto
    ...
    moreover {
      ...
      then have "\<exists>x \<in> {k<..i}. mem I (\<tau> \<sigma> i - \<tau> \<sigma> x) \<and> sat \<sigma> V v x (And \<phi> \<psi>)"
        using mem trigger k_props sat_trigger_rewrite_0_exist[of I \<sigma> V v i \<phi> \<psi> k]
        by auto
      then obtain x where x_props: "x \<in> {k<..i} \<and> mem I (\<tau> \<sigma> i - \<tau> \<sigma> x) \<and> sat \<sigma> V v x (And \<phi> \<psi>)" by blast
      then have "x \<in> A" using A_def by auto
      then have "x \<le> Max A" using A_nonempty A_finite by auto
      then have "x \<le> j \<and> x > k \<and> k > j" using j_def k_props x_props by auto
      then have "False" by auto
    }
    ultimately have "sat \<sigma> V v i (Or (Since \<psi> I (And \<phi> \<psi>)) (historically I \<psi>))" by blast
  }
...
next
  ...
  moreover {
    ...
    then have "sat \<sigma> V v i (Trigger \<phi> I \<psi>)" by auto
  }
  moreover {
    fix j
    assume since_and_j_props: "sat \<sigma> V v i (Since \<psi> I (And \<phi> \<psi>))" "j \<le> i" "mem I (\<tau> \<sigma> i - \<tau> \<sigma> j)"
    then obtain "j'" where j'_props: "j'\<le>i \<and> mem I (\<tau> \<sigma> i - \<tau> \<sigma> j') \<and> sat \<sigma> V v j' (And \<phi> \<psi>) \<and> (\<forall>k \<in> {j' <.. i}. sat \<sigma> V v k \<psi>)" by fastforce
    moreover {
      assume le: "j' < j"
      then have "\<forall>k \<in> {j' <.. i}. sat \<sigma> V v k \<psi>" using j'_props by auto
      then have "sat \<sigma> V v j \<psi>" using since_and_j_props le by auto
    ...
    moreover {
      ...
      moreover {
        assume "j = j'"
        from this j'_props have "sat \<sigma> V v j \<psi>" by auto
        then have "sat \<sigma> V v j \<psi> \<or> (\<exists>k \<in> {j <.. i}. sat \<sigma> V v k \<phi>)" by auto
      }
      moreover {
        assume neq: "j \<noteq> j'"
        then have "j' > j" using geq by auto
        then have "\<exists>k \<in> {j <.. i}. sat \<sigma> V v k \<phi>" using j'_props by auto
        then have "sat \<sigma> V v j \<psi> \<or> (\<exists>k \<in> {j <.. i}. sat \<sigma> V v k \<phi>)" by auto
      }
  ...
  moreover {
    ...
    (* helper lemma *)
    {
      ...
      assume x_props: "x\<ge>j" "\<not>mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> x)"
      {
        ...
        moreover have "\<tau> \<sigma> i - \<tau> \<sigma> j \<le> b" using assms j_props by auto 
        ultimately have "(\<tau> \<sigma> i - \<tau> \<sigma> x) \<le> b" using enat_ord_simps(1) order_trans by blast
        moreover have "(\<tau> \<sigma> i - \<tau> \<sigma> x) \<ge> a" using k_not_mem_1 x_props assms by auto
      ...
      moreover {
        ...
        moreover {
          ...
          then have k_cond: "k\<le>c \<and> sat \<sigma> V v k (And \<phi> \<psi>)" using k_sat by auto
          {
            ...
            then have lower: "(\<tau> \<sigma> i - \<tau> \<sigma> x) \<ge> (\<tau> \<sigma> i - \<tau> \<sigma> c)" by auto
            have "\<tau> \<sigma> x \<ge> \<tau> \<sigma> k"  using x_props by auto
            then have upper: "(\<tau> \<sigma> i - \<tau> \<sigma> x) \<le> (\<tau> \<sigma> i - \<tau> \<sigma> k)" by auto
            then have "(\<tau> \<sigma> i - \<tau> \<sigma> x) \<le> b"
          ...
          moreover {
            ...
            moreover {
              ...
              {
                assume "\<not>mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> (c+1))"
                then have upper: "(\<tau> \<sigma> i - \<tau> \<sigma> (c+1)) > b" using c_suc_not_mem_1 assms by auto
                ...
                moreover have "(\<tau> \<sigma> i - \<tau> \<sigma> c) \<le> b" using c_props assms by auto
                ultimately have "\<tau> \<sigma> i - \<tau> \<sigma> (c+1) \<le> b" using enat_ord_simps(1) order_trans by blast
                then have "False" using upper by auto
              }
              then have "mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> (c+1))" by blast
              then have "(c+1)\<le>i \<and> mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> (c+1)) \<and> sat \<sigma> V v (c+1) (Prev all (Since \<psi> all (And \<phi> \<psi>)))"
  ...
  moreover {
    ...
    then obtain j where j_props: "j\<le>i" "mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> j)" "sat \<sigma> V v j \<phi>" by blast
    {
      ...
      assume x_props: "x\<le>i" "mem I1 (\<tau> \<sigma> i - \<tau> \<sigma> x)"
      {
        ...
        then have "mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> x)" using j_props assms by auto
      ...
      moreover {
        ...
        moreover {
          ...
          have j_pred_psi: "sat \<sigma> V v (j-1) \<psi>" using j_pred_sat by auto
          {
            ...
            then have "mem I2 (\<tau> \<sigma> i - \<tau> \<sigma> l)" using assms j_props by auto
...
lemma sat_release_rewrite_0_mem:
  ...
  shows "sat \<sigma> V v j \<psi> \<or> (\<exists>k \<in> {i ..< j}. sat \<sigma> V v k (And \<phi> \<psi>))"
...
lemma sat_release_rewrite_0_exist:
  assumes mem: "mem I 0"
  assumes release: "sat \<sigma> V v i (Release \<phi> I \<psi>)"
  assumes j_props: "j\<ge>i \<and> mem I (\<tau> \<sigma> j - \<tau> \<sigma> i) \<and> \<not>sat \<sigma> V v j \<psi>"
shows "\<exists>k \<in> {i..<j}. mem I (\<tau> \<sigma> k - \<tau> \<sigma> i) \<and> sat \<sigma> V v k (And \<phi> \<psi>)"
proof -
  from assms have "\<exists>k\<in>{i..<j}. sat \<sigma> V v k (And \<phi> \<psi>)"
    using sat_release_rewrite_0_mem[of I \<sigma> V v i \<phi> \<psi> j]
    by auto
  then obtain k where k_props: "k \<in> {i..<j} \<and> sat \<sigma> V v k (And \<phi> \<psi>)" by blast
  from this mem j_props have "mem I (\<tau> \<sigma> k - \<tau> \<sigma> i)"
    using interval_leq[of I 0 \<sigma> j i k]
    by auto
  from this k_props show ?thesis by auto
qed
...
proof (rule iffI)
  ...
  moreover {
    ...
    have A_nonempty: "A\<noteq>{}"
      using mem release j'_props A_def sat_release_rewrite_0_exist[of I \<sigma> V v i \<phi> \<psi> j']
      by auto
    moreover have A_finite: "finite A" using A_def by auto
    ultimately have j_in_A: "j \<in> A"
      using j_def Max_in[of "A"]
      by auto
    then have j_props: "j\<in>{i..<j'} \<and> mem I (\<tau> \<sigma> j - \<tau> \<sigma> i) \<and> sat \<sigma> V v j (And \<phi> \<psi>)"
      using A_def
      by auto
    moreover {
    ...
    moreover {
      ...
      then have "\<exists>x \<in> {i..<k}. mem I (\<tau> \<sigma> x - \<tau> \<sigma> i) \<and> sat \<sigma> V v x (And \<phi> \<psi>)"
        using mem release k_props sat_release_rewrite_0_exist[of I \<sigma> V v i \<phi> \<psi> k]
        by auto
      then obtain x where x_props: "x \<in> {i..<k} \<and> mem I (\<tau> \<sigma> x - \<tau> \<sigma> i) \<and> sat \<sigma> V v x (And \<phi> \<psi>)" by blast
      then have "x \<in> A" using A_def j_props k_props by auto
      then have "x \<ge> Min A" using A_nonempty A_finite by auto
      then have "x \<ge> j \<and> x < k \<and> k < j" using j_def k_props x_props by auto
      then have "False" by auto
    }
...
next
  ...
  moreover {
    ...
    then obtain "j'" where j'_props: "j'\<ge>i \<and> mem I (\<tau> \<sigma> j' - \<tau> \<sigma> i) \<and> sat \<sigma> V v j' (And \<phi> \<psi>) \<and> (\<forall>k \<in> {i ..< j'}. sat \<sigma> V v k \<psi>)" by fastforce
    moreover {
      assume ge: "j' > j"
    ...
    moreover {
      ...
      moreover {
        assume neq: "j \<noteq> j'"
        then have "j' < j" using leq by auto
        then have "\<exists>k \<in> {i ..< j}. sat \<sigma> V v k \<phi>" using j'_props by auto
        then have "sat \<sigma> V v j \<psi> \<or> (\<exists>k \<in> {i ..< j}. sat \<sigma> V v k \<phi>)" by auto
      }
...
proof (rule iffI)
  ...
  moreover {
    ...
    then obtain j where j_props: "j\<ge>i \<and> mem I1 (\<tau> \<sigma> j - \<tau> \<sigma> i) \<and> \<not>sat \<sigma> V v j \<psi>" by auto
    ...
    then have A_props: "A \<noteq> {} \<and> finite A" using A_def by auto
    then have "k \<in> A" using k_def by auto
    then have k_props: "k \<in>{i ..< j} \<and> sat \<sigma> V v k \<phi>" using A_def by auto
    (* helper lemma *)
    {
      fix x
    ...
    moreover {
      ...
      moreover {
        ...
        then obtain l where l_props: "l \<in> {i ..< k} \<and> sat \<sigma> V v l \<phi>" by blast
        then have "l \<in> A" using A_def k_props l_props by auto
        then have "False" using A_props l_props k_def by auto
      }
      ultimately have "sat \<sigma> V v k \<psi>" by auto
      ...
      moreover {
        ...
        then have k_pred_geq_i: "k-1\<ge>i" using k_props by auto
        {
          ...
          moreover {
            ...
            then obtain l where l_props: "l \<in> {i ..< x} \<and> sat \<sigma> V v l \<phi>" by blast
            then have "l \<in> A" using A_def x_props k_props by auto
        ...
        moreover {
          ...
          then have B_props: "B \<noteq> {} \<and> finite B"
            ...
            by auto
          then have "c \<in> B" using c_def by auto
          then have c_props: "c\<in>{i..<k} \<and> mem I1 (\<tau> \<sigma> c - \<tau> \<sigma> i) \<and> sat \<sigma> V v c \<psi>" using B_def by auto
          then have k_cond: "k\<ge>c \<and> sat \<sigma> V v k (And \<phi> \<psi>)" using k_sat by auto
          {
            assume "c=0"
