      ...
      next
        ...
        proof (cases "case \<psi> of Formula.Trigger \<phi>' I \<psi>' \<Rightarrow> safe_formula_dual True safe_formula \<phi>' I \<psi>' | _ \<Rightarrow> False")
          ...
          then show ?thesis 
            ...
            by (clarsimp simp add: safe_formula_dual_def)
              (metis le_add2)
        next
          case False
...
next
  ...
  then show ?case
    by (auto elim!: safe_regex_mono[rotated] simp: case_Neg_iff regex.pred_set)
qed (auto simp add: is_simple_eq_def list.pred_set case_Neg_iff safe_formula_dual_def split: if_splits)
  ...
  next
    ...
    proof (cases "\<exists>\<phi>' I \<psi>'. \<psi> = Formula.Trigger \<phi>' I \<psi>'")
      ...
      then show ?thesis
        ...
        unfolding mmonitorable_def
        by (auto simp add: safe_assignment_def safe_formula_dual_def 
...
next
  ...
  proof (cases "mem I 0")
    ...
    then show ?thesis
      using 15
      by (auto simp add: case_Neg_iff mmonitorable_def safe_formula_dual_def split: if_splits)
  next
    case False
    then show ?thesis
      using 15
...
lemma mformula_induct[case_names MRel MPred MLet MLetPast MAnd MAndAssign MAndRel MAnds MOr MNeg
  ...
  "(\<And>rel. P (MRel rel)) \<Longrightarrow>
   ...
   (\<And>args A_\<phi> \<phi> \<phi>' \<psi>' tbuf buf nts aux. P \<phi> \<Longrightarrow> P (MAndTrigger A_\<phi> \<phi> tbuf args \<phi>' \<psi>' buf nts aux)) \<Longrightarrow>
...
| "progress P (Formula.Trigger \<phi> I \<psi>) j = min (progress P \<phi> j)
    (if memL I 0 then progress P \<psi> j else Suc (progress P \<psi> j))"
...
inductive (in maux) wf_mformula :: "Formula.trace \<Rightarrow> nat \<Rightarrow> _ \<Rightarrow> _ \<Rightarrow> nat 
  ...
  | MatchP: "(case to_mregex r of (mr', \<phi>s') 
    ...
    \<Longrightarrow> mrs = sorted_list_of_set (RPDs mr) \<Longrightarrow> safe_regex Past Strict r
  ...
  | MatchF: "(case to_mregex r of (mr', \<phi>s') 
      \<Rightarrow> list_all2 (wf_mformula \<sigma> j P V n R) \<phi>s \<phi>s' \<and> mr = mr')
    \<Longrightarrow> mrs = sorted_list_of_set (LPDs mr) \<Longrightarrow> safe_regex Futu Strict r
    \<Longrightarrow> wf_mbufn' \<sigma> P V j n R r buf \<Longrightarrow> wf_ts_regex \<sigma> P j r nts
...
proof (induction \<phi> arbitrary: db P P' V n R \<phi>' j \<delta> S E rule: mformula_induct)
  case (MRel rel) (* other branches have more cases *)
  let ?case1 = "\<lambda>t\<^sub>1 t\<^sub>2. \<phi>' = (Formula.Eq t\<^sub>1 t\<^sub>2) \<and> rel = eq_rel n t\<^sub>1 t\<^sub>2 \<and> 
    is_simple_eq t\<^sub>1 t\<^sub>2 \<and> (\<forall>x\<in>fv_trm t\<^sub>1. x < n) \<and> (\<forall>x\<in>fv_trm t\<^sub>2. x < n)" 
...
next
  ...
  let ?p = "(p, length ts)"
  have inits: "\<phi>' = Formula.Pred p ts" "\<forall>x\<in>fv (Formula.Pred p ts). x < n" 
  ...
  have wfmf: "wf_mformula \<sigma> (j + \<delta>) P' V n R (MPred p ts mode) (Formula.Pred p ts)"
  ...
  proof (cases "?p \<in> dom P") \<comment> \<open>notice @{term "dom P = dom V"}\<close>
    ...
    hence case1: "is_copy_pattern n ts 
      \<Longrightarrow> list_all2 (\<lambda>i. qtable n (Formula.fv \<phi>') (mem_restr R) 
        (\<lambda>v. Formula.sat \<sigma> V (map the v) i \<phi>')) [m..<m'] Rs"
      using qtable_copy_pattern(1)
      by - (erule list.rel_mono_strong, clarsimp simp: inits)
    moreover have case2: "is_simple_pattern ts 0 
      \<Longrightarrow> list_all2 (\<lambda>i T. qtable n (Formula.fv \<phi>') (mem_restr R) 
        (\<lambda>v. Formula.sat \<sigma> V (map the v) i \<phi>') (simple_match n 0 ts ` T)) [m..<m'] Rs"
    ...
    moreover have case3: "\<not> is_copy_pattern n ts \<Longrightarrow> \<not> is_simple_pattern ts 0 
      \<Longrightarrow> list_all2 (\<lambda>i T. qtable n (Formula.fv \<phi>') (mem_restr R) 
        (\<lambda>v. Formula.sat \<sigma> V (map the v) i \<phi>') 
